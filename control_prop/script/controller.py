...
class FanControl:
    ...
    POWER_LIMIT_DIGITAL = 0.5
    POWER_LIMIT_ANALOG = 0.5

    MOTOR_MAP = {
        "forward": [0, 2, 5, 7],  # forward
        "left":    [4, 5, 6, 7],  # left
        "back":    [1, 3, 4, 6],  # back
        "right":   [0, 1, 2, 3],  # right

        "turn_right": [1, 3, 5, 7],  # turn_right
        "turn_left":  [0, 2, 4, 6],  # turn_left
    ...
    def drive_motor(self, motor, power_ratio):
        duty = int( self.DUTY_MIN + (self.DUTY_MAX-self.DUTY_MIN) * power_ratio + 30)
...
#        スリープ防止。ときどき弱く入力を入れる。たまに回っちゃうからないほうがいいかも。
